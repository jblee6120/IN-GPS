...
%필터링 된 전류데이터 plot
figure(1);
set(figure(1),'Position',[0,0,770,800])
subplot(2,2,1);
...
figure(2);
set(figure(2),'Position',[770,0,770,800])
subplot(2,2,1);
plot(time, w_ref, time, w_real);
title('PI speed control result');
xlabel('time[sec]'); ylabel('speed[rad/sec]');
ylim([-5 105]);
legend('speed ref.', 'real speed');
...
set_param('pi_control_sim/Manual Switch1', 'sw', '0');
sim("pi_control_sim.slx");
subplot(2,2,3);
plot(tout, i_sim_ref, tout, i_sim_out);
title('PI current control result');
xlabel('time[sec]'); ylabel('current[A]');
ylim([-0.01 0.06]);
legend('current ref.', 'current sim.');

set_param('pi_control_sim/Manual Switch1', 'sw', '1');
sim("pi_control_sim.slx");
subplot(2,2,4);
plot(tout, i_sim_ref, tout, i_sim_out);
title('PI current control result');
